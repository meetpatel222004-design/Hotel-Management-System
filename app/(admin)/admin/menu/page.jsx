...
function AddItemModal({ open, onClose, onAdd }) {
  ...
  const handleImageUpload = (e) => {
    ...
    reader.onload = (ev) => { setImagePreview(ev.target.result); setImageData(ev.target.result); };
        ...
        <div>
          ...
          <div className="grid grid-cols-4 gap-2">
            {SPICY_LEVELS.map((level) => (
              <button key={level.value} type="button" onClick={() => setSpicy(level.value)}
                className={`rounded-2xl py-2.5 text-xs font-semibold border transition text-center ${spicy === level.value ? "border-primary/50 bg-primary/10 text-primary" : "border-border glass text-muted-foreground"}`}>
                {level.value > 0 && (
                  <span className="flex items-center justify-center gap-0.5 mb-0.5">
                    {Array.from({ length: level.value }).map((_, i) => <Flame key={i} className="h-3 w-3" />)}
                  </span>
                )}
                {level.label}
              </button>
        ...
        <div>
          ...
          {imagePreview ? (
            <div className="relative">
              ...
              <button type="button" onClick={() => { setImagePreview(null); setImageData(""); }}
                className="absolute top-2 right-2 h-7 w-7 rounded-full bg-black/60 text-white grid place-items-center text-sm hover:bg-black/80 transition">&times;</button>
            </div>
          ) : (
          ...
          {!imagePreview && <p className="text-xs text-muted-foreground mt-1.5">Default image used if none uploaded.</p>}
        </div>
